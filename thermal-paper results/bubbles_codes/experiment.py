...
nLat     = 1024
nTimeMAX = 5*nLat
bytesUNDECAYED = 414187840

phi0List = np.array([2.*np.pi/4.5, 2.*np.pi/4.5, 2.*np.pi/4.5, 2.*np.pi/4.5, 2.*np.pi/4.5, 2.*np.pi/4.5])
tempList = np.array([0.09, 0.10, 0.11, 0.12, 0.13, 0.14])
lambList = np.array([1.5, 1.5, 1.5, 1.5, 1.5, 1.5])
...
changed_sim_location = lambda nL,la,ph,te,sim: root_dir + 'bubbles_dat/' + batch_params(nL,la,ph,te) + '_change_sim'+str(sim) + '_fields.npy'
...
varians_tlist_file = lambda nL,la,ph,te,minS,maxS: root_dir + batch_params(nL,la,ph,te) + sims_interval(minS,maxS) + '_variances.npy'
...
def get_model(tmp):
    if tmp == 0: minSim, maxSim = 0, 4000
    elif tmp == 1: minSim, maxSim = 0, 4000
    elif tmp == 2: minSim, maxSim = 0, 4000
    elif tmp == 3: minSim, maxSim = 0, 4000
    elif tmp == 4: minSim, maxSim = 0, 4000
    elif tmp == 5: minSim, maxSim = 0, 4000
    else:
    ...
    return phi0, temp, lamb, sigmafld, minSim, maxSim, right_Vmax

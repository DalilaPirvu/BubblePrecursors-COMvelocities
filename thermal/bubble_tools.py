...
def simple_imshow(simulation, extent_floats, title='Sim', cmp='PRGn'):
    ...
    im0 = plt.imshow(simulation, aspect='auto', interpolation='none', extent=extent_floats, origin='lower', cmap=cmp)
...
def find_COM_vel(real, ampList, xList, nL, light_cone, phi_init, crit_thresh, crit_rad, dx, plots):
    real = real[0]
    nT, nN = np.shape(real)
...
def get_COM_velocity(simulation, phi_init, crit_thresh, crit_rad, t_cen, t_stop, vvv, xxx, dx, plots):
 ...
 #   print(t_centre, x_centre, tl, tr, xl, xr)
    ...
    if plots:
        ...
        win = 50
        ext = [max(0, x_centre-win)*dx, min(nN-1, x_centre+win)*dx, max(0, t_centre-win)*dx, min(nT-1, t_centre+win)*dx]
        sni2plot = simulation[max(0,t_centre-win): min(nT-1,t_centre+win), max(0,x_centre-win): min(nN-1,x_centre+win)]
        im0 = ax0.imshow(sni2plot, aspect='auto', interpolation='none', extent=ext, origin='lower', cmap='viridis', alpha=0.8)
        clb = plt.colorbar(im0, ax = ax0)
        ...
        ax0.plot((rrwallfit+xl+1)*dx, ttwallfit*dx, color='blueviolet', ls='-.', linewidth=2)
        ax0.plot((llwallfit+xl+1)*dx, ttwallfit*dx, color='orangered', ls='-.', linewidth=2)

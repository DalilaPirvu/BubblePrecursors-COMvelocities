...
program Gross_Pitaevskii_1d
  ...
  !>        (2) thermal+vacuum
  !>        (3) only thermal
  real(dl), parameter :: phi0 = twopi / 3.5_dl
  real(dl), parameter :: temp = 0.11
  integer, parameter :: type = 3

  integer :: nTimeMax = 32768
  integer :: sim
  integer :: lSim = 0, nSim = 100
...
!    potential(:) = 4._dl*nu * (-cos(fld(:,1)) + 0.5_dl*lambda**2._dl * sin(fld(:,1))**2._dl)
    ...
    inquire(file='/gpfs/dpirvu/velocity_comparison/x'//trim(str(nLat))//'_phi0'//trim(real_str(phi0))//'_lambda'//trim(real_str(lambda))//'_T'//trim(real_str(temp))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)

    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/velocity_comparison/x'//trim(str(nLat))//'_phi0'//trim(real_str(phi0))//'_lambda'//trim(real_str(lambda))//'_T'//trim(real_str(temp))//'_sim'//trim(str(sim))//'_fields.dat')
